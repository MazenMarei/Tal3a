...
impl CityData {
    ...
    pub fn get_by_id(city_id: u16, governorate_id: u8) -> Result<CityData, String> {
        let cities_data: Value = serde_json::from_str(CITIES_JSON).map_err(|_| "Failed to parse cities JSON".to_string())?;

        // * Navigate to the governorate
        let governorate_name = GovernorateData::get_by_id(governorate_id).map_err(|_| "Failed to get governorate".to_string())?;

        let governorate_cities = cities_data.get(&&governorate_name.name.to_string()).ok_or("Governorate not found".to_string())?;

        // * Get the city by ID
        let city_value = governorate_cities.get(&city_id.to_string()).ok_or("City not found".to_string())?;
        ...
        let city_data: CityData = serde_json::from_value(city_value.clone()).map_err(|_| "Failed to parse city data".to_string())?;
        Ok(city_data)
    }
