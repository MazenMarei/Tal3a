...
impl Post {
    pub async fn new(post: NewPost) -> Result<Post, Error> {
        ...
        if !group.iter().any(|member| member.user_id == msg_caller()) {
            return Err(Error {
                code: 403,
                error: "User is not a member of the group".into(),
        ...
        if post_id.is_err() {
            return Err(Error {
                code: 500,
                error: "Failed to generate unique post ID".into(),
                message: "Failed to generate unique post ID".to_string(),
            });
        }
    ...
    pub fn get_by_id(post_id: &str) -> Result<Post, Error> {
        POSTS.with(|posts| {
            posts
                .borrow()
    ...
    pub fn delete(post_id: &str) -> Result<(), Error> {
        ...
        if post.author != msg_caller() {
            return Err(Error {
                ...
                message: "Only post author can delete the post".to_string(),
            });
        }
    ...
    pub fn update(post_id: &str, updated_post: UpdatePost) -> Result<Post, Error> {
        ...
        if post.author != msg_caller() {
            return Err(Error {
                code: 403,
                error: "Unauthorized".into(),
                message: "Only post author can update the post".to_string(),
            });
        }
    ...
    pub fn mark_as_read(&self, user_id: Principal) -> Result<(), Error> {
        UNSEEN_POSTS.with(|unseen_posts| {
            ...
            if let Some(posts) = &mut unseen_posts.get(&user_id) {
                ...
                Err(Error {
                    ...
                    message: "User has no unseen posts".to_string(),
                })
            }
        })
    }
