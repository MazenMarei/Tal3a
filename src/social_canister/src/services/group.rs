...
impl Group {
    pub async fn new(new_group: CreatingGroup) -> Result<Self, Error> {
        ...
        if let Some(parent_group_id) = new_group.parent_group_id.clone() {
            ...
            if let Err(e) = parent_group {
                ...
                return Err(Error {
                    code: 400,
                    error: "Cannot create a club under another club".to_string(),
                    message: "Cannot create a club under another club".to_string(),
                });
            } else {
                ...
                if !is_member {
                    return Err(Error {
                        code: 403,
                        error: "User is not a member of the parent group".to_string(),
                        message: "User is not a member of the parent group".to_string(),
                    });
                }
            }
        }
        ...
        if Self::get_by_id(&new_id.clone()).is_ok() {
            return Err(Error {
                code: 409,
                error: format!("Group / Club already exists with ID {}", new_id),
                message: "Group / Club already exists with the given ID".to_string(),
            });
        }
    ...
    pub fn get_by_id(group_id: &str) -> Result<Self, Error> {
        GROUPS.with(|groups| {
    ...
    pub fn join(&self, user: Principal) -> Result<(), Error> {
        ...
        let group_members_result = GROUP_MEMBERS.with(|group_memb| {
            ...
            if let Some(group_members) = &mut group_members_list {
                if group_members.members.iter().any(|m| m.user_id == user) {
                    ...
                    return Err(Error {
                        code: 409,
                        error: format!("User {} is already a member of group {}", user, group_id),
                        message: "User is already a member of the group".to_string(),
                    });
                } else {
                ...
                return Err(Error {
                    code: 404,
                    error: format!("Cannot find members list for group {}", group_id),
                    message: "No members list exists for the given group".to_string(),
                });
            }
    ...
    pub fn leave(&self, user: Principal) -> Result<(), Error> {
        GROUP_MEMBERS.with(|members| {
            ...
            if let Some(group) = &mut group {
                if let Some(pos) = group.members.iter().position(|m| m.user_id == user) {
                    ...
                    Err(Error {
                        code: 404,
                        error: format!("User {} is not a member of group {}", user, group_id),
    ...
    pub fn delete_internal(&self, parent_delete: Option<bool>) -> Result<(), Error> {
        GROUPS.with(|groups| {
            let group_id = self.id.clone();
                    ...
                    || parent_delete == Some(true) && parent_group.unwrap().created_by != user)
            {
                return Err(Error {
                    ...
                    message: "Only the group creator can delete the group".to_string(),
                });
            }
