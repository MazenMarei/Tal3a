...
#[update]
pub async fn create_group(new_group: CreatingGroup) -> Result<Group, Error> {
    Group::new(new_group).await
}

#[query]
pub fn get_group(group_id: &str) -> Result<Group, Error> {
...
#[update]
pub async fn join_group(group_id: &str) -> Result<(), Error> {
    let user = msg_caller();
...
#[update]
pub fn leave_group(group_id: &str) -> Result<(), Error> {
    let user = msg_caller();
    let group = Group::get_by_id(group_id)?;
    group.leave(user)
}
