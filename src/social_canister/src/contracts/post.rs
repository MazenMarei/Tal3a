...
#[update]
pub async fn create_post(new_post: NewPost) -> Result<Post, String> {
    Post::new(new_post).await
}

#[query]
pub async fn get_post(post_id: &str) -> Result<Post, String> {
...
#[update]
pub async fn mark_post_as_read(post_id: &str) -> Result<(), String> {
    let user_id = msg_caller();
    Post::get_by_id(post_id)?.mark_as_read(user_id)
}

#[update]
pub async fn update_post(post_id: &str, updated_post: UpdatePost) -> Result<Post, String> {
