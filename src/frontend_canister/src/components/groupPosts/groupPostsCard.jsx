...
const GroupPostsCard = () => {
  ...
  const currentGroup = groups?.find(
    (group) => group.id === groupId || group.id === parseInt(groupId)
  );
  ...
  const groupPosts =
    posts?.filter(
      (post) => post.groupId === groupId || post.groupId === parseInt(groupId)
    ) || [];
  ...
  const handleJoinGroup = () => {
    setIsJoined(true);
    toast.success(
      `Congratulations! You have successfully joined ${currentGroup.name}! 🎉`,
      {
        duration: 4000,
        position: "top-center",
        style: {
  ...
  const recentPosts = [
    ...
    },
    {
      id: 2,
      author: "Sara Mohamed",
      content: "Looking forward to tomorrow's match!",
      time: "5 hours ago",
    },
    {
      id: 3,
      author: "Omar Ali",
      content: "New training schedule is up!",
      time: "1 day ago",
    },
    {
      id: 4,
      author: "Fatma Nour",
      content: "Thanks for the amazing game everyone! 🏆",
      time: "2 days ago",
    },
  ];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={`View posts and discussions from ${currentGroup.name} sports group. ${currentGroup.sportType} group in ${currentGroup.location}.`}
        />
        <meta
          name="keywords"
          content={`${currentGroup.name}, ${currentGroup.sportType}, ${currentGroup.location}, sports group, Egypt sports`}
        />
        <meta
          property="og:title"
          content={`${currentGroup.name} - Group Posts`}
        />
        <meta
          property="og:description"
          content={`Join the conversation in ${currentGroup.name} - ${currentGroup.sportType} group`}
        />
            ...
            <div className="absolute top-4 right-4">
              <div className="bg-white/90 backdrop-blur-sm rounded-full p-2 shadow-lg">
                <span className="text-2xl">
                  {getSportIcon(currentGroup.sportType)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
              <div>
                ...
                <Badge
                  ...
                  )} font-medium text-sm px-3 py-1 border mb-4`}
                >
                  {currentGroup.skillLevel}
                </Badge>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {currentGroup.location || "Egypt"}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 text-teal-500" />
                <span className="font-medium">
                  {currentGroup.memberCount || 10} members
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Trophy className="h-4 w-4 text-teal-500" />
                <span className="font-medium">
                  {currentGroup.sportType || "Gold"}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Gift className="h-4 w-4 text-teal-500" />
                <span className="font-medium">
                  {currentGroup.prize || "Champion"}
                </span>
              </div>
            </div>
          </CardContent>
                  ...
                  <CardHeader className="p-6 border-b border-teal-50">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <Badge
                          variant="outline"
                          className="text-xs mt-1 text-teal-600 border-teal-200"
                        >
              ...
              <CardContent className="p-4 pt-0">
                <div className="space-y-4">
                  {recentPosts.slice(0, 4).map((post) => (
                    <div
                      key={post.id}
                      className="border-l-2 border-teal-200 pl-3 py-2"
                    >
