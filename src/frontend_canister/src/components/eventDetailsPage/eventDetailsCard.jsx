...
const EventDetailsCard = () => {
  ...
  const currentEvent = events?.find(
    (event) => event.id === eventId || event.id === parseInt(eventId)
  );
  ...
  const eventPosts =
    posts?.filter(
      (post) => post.eventId === eventId || post.eventId === parseInt(eventId)
    ) || [];

  // Find participation data for this event
  const eventParticipation = participationData.find(
    (part) => part.eventId === eventId || part.eventId === parseInt(eventId)
  );
  ...
  const handleBookmark = () => {
    ...
    toast.success(
      isBookmarked ? "Event removed from bookmarks" : "Event bookmarked!",
      {
        duration: 2000,
        position: "top-center",
      }
      ...
      : [
          {
            id: 1,
            author: "Ahmed Ali",
            content: "Can't wait for this event! 🏃‍♂️",
            time: "1 hour ago",
          },
          {
            id: 2,
            author: "Sara Hassan",
            content: "Who else is going? Let's meet up!",
            time: "3 hours ago",
          },
          {
            id: 3,
            author: "Omar Mohamed",
            content: "This looks amazing! Count me in! 🔥",
            time: "5 hours ago",
          },
          {
            id: 4,
            author: "Fatma Ahmed",
            content: "Perfect timing for the weekend! ⚽",
            time: "1 day ago",
          },
        ];
  ...
  return (
    <>
      <Helmet>
        <title>
          {currentEvent.title} - Event Details | Egypt Sports Community
        </title>
        <meta
          name="description"
          content={`Join ${currentEvent.title} - ${currentEvent.sport} event in ${currentEvent.location}. ${currentEvent.going} people attending.`}
        />
        <meta
          name="keywords"
          content={`${currentEvent.title}, ${currentEvent.sport}, ${currentEvent.location}, sports event, Egypt sports`}
        />
        <meta
          property="og:title"
          content={`${currentEvent.title} - Sports Event`}
        />
        <meta
          property="og:description"
          content={`Join us for ${currentEvent.title} in ${currentEvent.location}`}
        />
          ...
          <CardContent className="p-6 sm:p-8 lg:p-10">
            {eventParticipation ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  ...
                  <span className="text-gray-700 text-sm">
                    {eventParticipation.description ||
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  ...
                  <span className="text-gray-700 text-sm">
                    {new Date(
                      eventParticipation.event_date * 1000
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  ...
                  <span className="text-gray-700 text-sm">
                    {eventParticipation.duration_hours} hours
                  </span>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  <span className="block text-sm font-semibold text-green-600 mb-1">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  ...
                  <span className="text-gray-700 text-sm">
                    {eventParticipation.max_participants || "Unlimited"}
                  </span>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  <span className="block text-sm font-semibold text-green-600 mb-1">
                  ...
                  <span className="text-gray-700 text-sm">
                    {eventParticipation.cost_per_person
                      ? `${eventParticipation.cost_per_person} EGP`
                      : "Free"}
                  </span>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  ...
                  <span className="text-gray-700 text-sm">
                    {eventParticipation.requirements.join(", ") || "None"}
                  </span>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow duration-200">
                  <span className="block text-sm font-semibold text-green-600 mb-1">
              ...
              <p className="text-gray-500 text-sm text-center">
                No participation details available for this event.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {eventPosts.length === 0 ? (
              ...
              eventPosts.map((post) => (
                <Card
                  key={post.id}
                  className="bg-white shadow-lg rounded-xl border border-green-100 hover:shadow-xl transition-shadow duration-300"
                >
                  <CardHeader className="p-6 border-b border-green-50">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <Badge
                          variant="outline"
                          className="text-xs mt-1 text-green-600 border-green-200"
                        >
                          Event Participant
                        </Badge>
                      </div>
              ...
              <CardContent className="p-4 pt-0">
                <div className="space-y-4">
                  {recentEventPosts.slice(0, 4).map((post) => (
                    <div
                      key={post.id}
                      className="border-l-2 border-green-200 pl-3 py-2"
                    >
