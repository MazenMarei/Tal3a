  ...
  return (
    <div className="flex w-full flex-col items-center justify-center px-0 md:px-4 pb-8 bg-light-green-50 min-h-screen sm:px-6 lg:px-8">
      <div className="flex justify-end w-full mb-2">
        <Link to="/createEvent">
          <Button className="w-50 bg-gradient-to-r my-4 from-green-600 to-green-500 hover:from-green-600 hover:to-green-500 text-white font-semibold py-3 px-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 group/btn cursor-pointer flex items-center justify-center gap-2 mx-auto">
                    ...
                    <div className="absolute top-2 sm:top-4 right-2 sm:right-4">
                      <Badge
                        className={`${getSportColor(
                          event.type
                        )} border-0 px-2 sm:px-3 py-1 text-xs sm:text-sm font-semibold shadow-lg`}
                      >
                        {event.type || "Unknown"}
                      </Badge>
                    </div>
                    ...
                    {eventTags.map((tag, tagIndex) => {
                      ...
                      if (tag === "View Details") {
                        return (
                          ...
                          >
                            <Button
                              size="sm"
                              className={`text-xs cursor-pointer sm:text-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 ${getButtonStyle(
                                tag
                              )}`}
                            >
                      ...
                      return (
                        <Button
                          key={tagIndex}
                          size="sm"
                          className={`text-xs sm:text-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 ${getButtonStyle(
                            tag
                          )}`}
                        >
                          {tag}
