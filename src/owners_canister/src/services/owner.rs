...
pub fn has_permission(principal: &Principal, permission: &Permission) -> bool {
    storage::get_owner(principal).map_or(false, |owner| {
...
pub fn add_owner(
    ...
    if !is_super_admin(caller) {
        return Err(Error::NotAuthorized("Only super admins can add owners".to_string()));
    }

    // Check if owner already exists
    if storage::owner_exists(&new_owner_principal) {
        return Err(Error::AlreadyExists("Principal is already an owner".to_string()));
    }
...
pub fn remove_owner(caller: &Principal, owner_principal: Principal) -> Result<(), Error> {
    // Check if caller is super admin
    if !is_super_admin(caller) {
        return Err(Error::NotAuthorized("Only super admins can remove owners".to_string()));
    }

    // Cannot remove self if you're the only super admin
    if owner_principal == *caller && storage::count_super_admins() <= 1 {
        return Err(Error::InvalidInput("Cannot remove the last super admin".to_string()));
    }
...
pub fn get_all_owners(caller: &Principal) -> Result<Vec<Owner>, Error> {
    ...
    if !storage::owner_exists(caller) {
        return Err(Error::NotAuthorized("Only owners can view the owners list".to_string()));
    }
...
pub fn get_owner_info(caller: &Principal) -> Result<Owner, Error> {
    storage::get_owner(caller)
        .ok_or_else(|| Error::NotFound("You are not an owner".to_string()))
}
...
pub fn update_owner_permissions(
    ...
    // Check if caller is super admin
    if !is_super_admin(caller) {
        return Err(Error::NotAuthorized("Only super admins can update owner permissions".to_string()));
    }
...
// Validate permissions based on role
fn validate_role_permissions(role: &OwnerRole, permissions: Vec<Permission>) -> Result<Vec<Permission>, Error> {
    match role {
        ...
        OwnerRole::Admin => {
            ...
            for perm in &permissions {
                if forbidden.contains(perm) {
                    return Err(Error::InvalidInput(format!("Admins cannot have {:?} permission", perm)));
                }
            }
            Ok(permissions)
        },
        OwnerRole::Moderator => {
            ...
            for perm in &permissions {
                if !allowed.contains(perm) {
                    return Err(Error::InvalidInput(format!("Moderators cannot have {:?} permission", perm)));
                }
            }
            Ok(permissions)
        },
