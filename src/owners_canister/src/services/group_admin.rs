use crate::types::{GroupAdmin, GroupPermission, Error};
use crate::storage;
...
pub fn has_group_permission(principal: &Principal, group_id: u64, permission: &GroupPermission) -> bool {
    storage::get_group_admin(group_id, principal).map_or(false, |admin| {
...
pub fn add_group_admin(
    ...
    if !has_permission {
        return Err(Error::NotAuthorized("Insufficient permissions to add group admin".to_string()));
    }

    // Check if already admin
    if storage::group_admin_exists(group_id, &admin_principal) {
        return Err(Error::AlreadyExists("Principal is already an admin of this group".to_string()));
    }
...
pub fn remove_group_admin(
    ...
    if !has_permission {
        return Err(Error::NotAuthorized("Insufficient permissions to remove group admin".to_string()));
    }
    ...
    if storage::remove_group_admin(group_id, &admin_principal) {
        ...
        Err(Error::NotFound("Group admin not found".to_string()))
    }
}
...
pub fn get_group_admins(
    ...
    if !has_permission {
        return Err(Error::NotAuthorized("Insufficient permissions to view group admins".to_string()));
    }
...
pub fn update_group_admin_permissions(
    ...
    if !has_permission {
        return Err(Error::NotAuthorized("Insufficient permissions to update group admin permissions".to_string()));
    }

    let mut admin = storage::get_group_admin(group_id, &admin_principal)
        .ok_or_else(|| Error::NotFound("Group admin not found".to_string()))?;
